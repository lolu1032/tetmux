...
// to one column. go-runewidth auto-enables EastAsianWidth from the locale (e.g.
// a Korean LANG), which would make box-drawing (─ │ ╭), bullets (·) and arrows
// two columns wide — but modern terminals and width-aware clients like Claude
// Code treat those as one. Pinning it keeps tetmux's grid in agreement with the
// client so only truly wide glyphs (Hangul, CJK, emoji) reserve two columns.
...
// runeCells returns how many terminal columns a rune occupies: 2 for East
// Asian wide / fullwidth glyphs (CJK, Hangul, etc.), 1 otherwise. tetmux patch:
// upstream vt10x always advanced the cursor by exactly one column per rune,
// which desynced the grid from width-aware clients (a Hangul syllable is two
// columns wide on a real terminal), leaving a blank "phantom" cell after every
// wide glyph. Advancing by the true width keeps the grid aligned.
func runeCells(c rune) int {
	if widthCond.RuneWidth(c) == 2 {
		return 2
	}
	return 1
}
...
func (t *State) parse(c rune) {
	...
	// Advance by the glyph's true display width so wide (CJK) glyphs reserve the
	// two columns they actually occupy. The reserved trailing column keeps its
	// prior (blank) content; the renderer skips it via the same width logic.
	w := runeCells(c)
	if t.cur.X+w < t.cols {
