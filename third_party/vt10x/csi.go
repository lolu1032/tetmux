...
func (t *State) handleCSI() {
	...
	case 'I': // CHT - cursor forward tabulation <n> tab stops
		// Clamp the repeat to the column count: a child emitting a huge count
		// (e.g. "\x1b[999999999I") would otherwise spin the reader goroutine
		// under the vt lock, freezing tetmux — a one-sequence CPU DoS. There are
		// at most t.cols tab stops to cross, so anything beyond that is a no-op.
	...
	case 'Z': // CBT - cursor backward tabulation <n> tab stops
		// Clamp the repeat to the column count (see CHT above): bounds the loop so
		// a crafted huge count can't spin the reader goroutine under the vt lock.
		n := clamp(c.arg(0, 1), 0, t.cols)
		for i := 0; i < n; i++ {
			t.putTab(false)
		}
